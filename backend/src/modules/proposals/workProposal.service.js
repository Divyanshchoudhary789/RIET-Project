...
const buildFilter = (user, query) => {
  ...
  // Department admin is always scoped — cannot be overridden by query params
...
const createWorkProposal = async (data, clusterManager) => {
  ...
  if (requirements.length !== data.requirementRefs.length) {
    const error = new Error(
      'One or more requirements are invalid or not in a submittable state (must be Submitted or Revised).'
    );
  ...
  // Use $set + $push together — valid as both are MongoDB update operators
  const timelineEntry = buildTimelineEntry(
    clusterManager,
    TIMELINE_ACTIONS.FORWARDED,
    'Included in Work Proposal'
  );
  await Requirement.updateMany(
    { _id: { $in: data.requirementRefs } },
    {
      $set: { status: DOCUMENT_STATUS.FORWARDED, workProposalRef: proposal._id },
...
/**
 * Cluster Manager revises and resubmits a rejected work proposal.
 */
