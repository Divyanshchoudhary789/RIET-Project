...
const AccountsDashboard = () => {
  ...
  const fetchAccountsData = async () => {
    ...
    try {
      ...
      const issued = pos.filter((p) => p.status === 'issued' || p.status === 'Issued');
      const received = pos.filter((p) => p.status === 'closed' || p.status === 'Closed' || p.status === 'received' || p.status === 'Received');

      setStats({
        approvedMemos: memos.length,
        totalPOs: pos.length,
        issuedPOs: issued.length,
    ...
    <div>
      ...
      <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', marginBottom: 24 }}>
        ...
        <div className="stat-card" onClick={() => navigate('/accounts/purchase-orders')} style={{ cursor: 'pointer' }}>
          ...
          <div className="stat-info">
            <span className="stat-label">Issued POs</span>
            <span className="stat-value">{stats.issuedPOs}</span>
          </div>
        </div>
        ...
        <div className="stat-card" onClick={() => navigate('/accounts/purchase-orders')} style={{ cursor: 'pointer' }}>
          ...
          <div className="stat-info">
            <span className="stat-label">Goods Received</span>
            <span className="stat-value">{stats.receivedPOs}</span>
          </div>
        </div>
      </div>

      {/* Approved Memos Queue */}
      <div className="section-card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid var(--color-border)' }}>
          ...
          <button className="btn btn-ghost" onClick={() => navigate('/accounts/memos')}>
                    ...
                    <td>
                      <div className="actions-cell">
                        ...
                        <button
                          ...
                          onClick={() => navigate('/accounts/memos')}
                        >
                          Generate PO
                        </button>
                      </div>
